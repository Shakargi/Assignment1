...
void MP3Track::analyze_beatgrid() {
    ...
    double precision_factor = static_cast<double>(bitrate) / 320.0;
    std::cout << "  → Estimated beats: " << beats 
            << "  → Compression precision factor: " << precision_factor 
            << " (lossy compression)" << std::endl;
...
double MP3Track::get_quality_score() const {
    ...
    double score = static_cast<double>(bitrate) / 320.0 * 100.0;
    if (has_id3_tags)
    {
        score += 5.0; // Slight boost for having metadata
    }
    if (bitrate < 128)
    {
        score -= 10.0; // Penalty for low bitrate
    }
...
PointerWrapper<AudioTrack> MP3Track::clone() const {
    ...
    return PointerWrapper<AudioTrack>(new MP3Track(*this)); // Deep copy via copy constructor
}